...
def overlappingArea(contours, contours2, shape):
    ...
    res, contours_, _ = cv2.findContours(res, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        
    res2, contours2_, _ = cv2.findContours(res2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    return overlapping, (overlapping/ cv2.countNonZero(res2) + overlapping/ cv2.countNonZero(res)) / 2
...
def findMaxOverlapping(contours, contours2, shape, vertices, num_it=5):
    ...
    for it in range(num_it):
        ...
        contours = best_M.dot(np.hstack([contours_copy + best_translation, ones]).T).T
        
        ones = np.ones(shape=(vertices.shape[0], 1))
        vertices = best_M.dot(np.hstack([vertices + best_translation, ones]).T).T
    
    _, score = overlappingArea(contours, contours2, shape)

    return scale_x, scale_y, translation, best_M, contours.astype(np.int32), score, vertices
...
for f in in_files:
    if f.split('.')[-1] in ['jpg', 'png']:
        ...
        img = read_img(f)
        img2 = read_img('wall2.png')
    
        original_img = cv2.imread('./ori.png')
        ...
        _, _, _, _, contours, score, vertices = findMaxOverlapping(contours, contours2, img.shape, vertices)
        ...
        for i in range(contours.shape[0]):
            if contours[i][0] < 0 or contours[i][0] >= img2.shape[0] or contours[i][1] < 0 or contours[i][1] >= img2.shape[1]:
                continue
